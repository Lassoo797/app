...
import { LayoutDashboard, Map as MapIcon, Receipt, Users, Car, Briefcase, MapPin, Settings as SettingsIcon, X, Moon, Sun, Plus, AlertTriangle, Menu } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4 md:p-6 text-slate-900 dark:text-slate-200 overflow-y-auto">
          {children}
        </div>
      </div>
    </div>
...
export const EntityListLayout = ({ title, onAdd, children, isAdding, addForm }: any) => (
    <div className="max-w-5xl mx-auto animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <button onClick={onAdd} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-lg shadow-blue-900/20 w-full sm:w-auto justify-center">
                <Plus size={18}/> Pridať
            </button>
        </div>
...
export const MobileHeader = ({ onOpenSidebar, title }: any) => (
    <div className="lg:hidden bg-white dark:bg-slate-900 text-slate-800 dark:text-white px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm border-b dark:border-slate-800">
        <div className="font-bold text-lg flex items-center gap-2">
            <span className="bg-blue-600 p-1 rounded-md text-white text-xs">CP</span>
            {title}
        </div>
        <button onClick={onOpenSidebar} className="p-2 -mr-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><Menu size={24} /></button>
    </div>
);

export const Sidebar = ({ activeTab, setActiveTab, darkMode, toggleDarkMode, isOpen, onClose }: any) => {
  const menuItems = [
  ...
  return (
    <>
        ...
        <div className={`
            fixed lg:static inset-y-0 left-0 z-50 w-64 bg-slate-900 dark:bg-slate-950 text-white flex flex-col h-screen shadow-xl border-r border-slate-800 transition-transform duration-300 transform
